...
@Suppress("unused")
val lawniconsCustomIconsPatch = resourcePatch(
    name = "Custom Icons",
    description = "Add custom icon mappings to appfilter.xml.",
    default = false
) {
    compatibleWith(LAWNICONS_COMPATIBILITY)

    // Users set this when patching - format: component|drawable|name (one per line)
    val customIcons by stringOption(
        key = "custom_icons",
        default = "",
        title = "Custom Icon Entries",
    ...
    execute {
        ...
        if (customIcons?.isBlank() != false) {
            println("Lawnicons: No custom icons defined, skipping")
            return@execute
        }
        ...
        document("assets/appfilter.xml").use { doc ->
            ...
            val entriesText = customIcons?.trim() ?: return@use
            val entries = entriesText
                .split("\n", ";")
                .map { it.trim() }
                .filter { it.isNotBlank() }

            println("Lawnicons: Processing ${entries.size} custom icon entry(ies)")
            ...
            println("Lawnicons: Successfully added $added custom icon mapping(s) to appfilter.xml")
        }
    }
}
